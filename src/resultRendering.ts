...
export function renderPollResult(input: {
  ...
  const { definition, result, t } = input;
  ...
  lines.push(renderOutcome(definition, result, labelByOptionId, t));
  return lines.join('\n');
}
...
function renderOutcome(
  ...
  if (result.purpose === 'decide') {
    if (result.outcome.status === 'selected') {
      return t(`${MESSAGE_PREFIX}.selected`, {
        options: optionList(labelByOptionId, result.outcome.selectedOptionIds)
      });
    }
    if (result.outcome.status === 'tie') {
      return t(`${MESSAGE_PREFIX}.tie`, {
        certainOptions: optionList(labelByOptionId, result.outcome.certainOptionIds),
        tiedOptions: optionList(labelByOptionId, result.outcome.tiedOptionIds),
  ...
  if (result.purpose === 'measure') {
    ...
    return t(`${MESSAGE_PREFIX}.measuredScale`, {
      medianOptions: optionList(labelByOptionId, result.outcome.analysis.medianOptionIds),
      modeOptions: optionList(labelByOptionId, result.outcome.analysis.modeOptionIds)
    });
  }
...
function optionList(labels: ReadonlyMap<string, string>, optionIds: readonly string[]): string {
  return optionIds.map((optionId) => requireOptionLabel(labels, optionId)).join(', ');
}
