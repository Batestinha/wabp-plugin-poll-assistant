...
test('assistant proposal template has one bold rule and configurable option rows', () => {
  const options = [
    renderPollTemplate({ kind: 'proposalOption', t, values: { ordinal: 1, label: '1', numericValue: 1, option: '1' } }),
    renderPollTemplate({ kind: 'proposalOption', t, values: { ordinal: 2, label: '2', numericValue: 2, option: '2' } })
  ].join('\n');
...
test('the archive retains every SQL migration and all declared translations and controls', () => {
  ...
  assert.equal(require('../package.json').version, metadata.version);
